...
class Constants:
	...
	func _load_autoloads() -> void:
		self.autoloads = ProjectSettings.get_setting(RustToolsSettings.AUTOLOADS_CONFIG)
...
func _make_constants_definition_file() -> String:
	...
	return (
		...
		+ _make_autoload_constants(new_constants.autoloads)
	)


static func _make_header() -> String:
	# HACK: we use `#![cfg_attr(any(), rustfmt::skip)]` instead of `#![rustfmt::skip]`
	# since custom inner attributes are unstable.
	return (
...
func _make_autoload_constants(autoloads: Dictionary) -> String:
	if autoloads.is_empty():
		return ""

	var autoloads_definition := "\npub mod autoloads {"

	for autoload_name: String in autoloads:
		if not autoloads[autoload_name][RustToolsAutoloadSettings.ENABLED_COLUMN]:
			continue

		var autoload_snake_name := autoload_name.to_snake_case()
		var class_path: String = autoloads[autoload_name][
			RustToolsAutoloadSettings.EXPORT_DECL_COLUMN
		]

		autoloads_definition += (
			"\n"
			+ "    pub fn {autoload_name}() -> godot::obj::Gd<{class_path}> {\n".format(
				{autoload_name = autoload_snake_name, class_path = class_path}
			)
			+ '        godot::tools::get_autoload_by_name("{autoload_name}")\n'.format(
