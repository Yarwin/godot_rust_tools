...
const GENERATED_CONSTANTS_FILE_PATHS := "rust_tools/config/generated_constants/generated_constants_file_paths"
const AUTOLOADS_CONFIG := "rust_tools/config/generated_constants/autoloads_config"


## Registers all project and editor settings.
static func register() -> void:
	_register_editor_setting(
		_CARGO_EXECUTABLE,
		_CARGO_EXECUTABLE_INITIAL_VALUE,
		TYPE_STRING,
		PROPERTY_HINT_GLOBAL_FILE,
		""
	)
	_register_editor_setting(
		_ENABLE_AUTORELOAD, _ENABLE_AUTORELOAD_INITIAL_VALUE, TYPE_BOOL, PROPERTY_HINT_NONE, ""
	)
	_register_editor_setting(
		_ENABLE_BUILD_BEFORE_RUN,
		_ENABLE_BUILD_BEFORE_RUN_INITIAL_VALUE,
		TYPE_BOOL,
		PROPERTY_HINT_NONE,
		""
	)
	...
	_register_project_setting(
		...
		"%d/%d:*.gdextension" % [TYPE_STRING, PROPERTY_HINT_FILE],
		true
	)
	_register_project_setting(
		GENERATED_CONSTANTS_FILE_PATHS,
		PackedStringArray(),
		TYPE_PACKED_STRING_ARRAY,
		PROPERTY_HINT_NONE,
		"",
	...
	_register_project_setting(AUTOLOADS_CONFIG, {}, TYPE_DICTIONARY, PROPERTY_HINT_NONE, "", true)
	ProjectSettings.set_as_internal(AUTOLOADS_CONFIG, true)


## Registers a single editor setting.
static func _register_editor_setting(
	name: String,
...
static func _register_project_setting(
	name: String,
	initial_value: Variant,
	type: Variant.Type,
	hint: PropertyHint,
...
static func _get_setting(
	name: String, initial_value: Variant, is_editor: bool, type: Variant.Type
) -> Variant:
	var result: Variant = (
		EditorInterface.get_editor_settings().get_setting(name)
		if is_editor
		else ProjectSettings.get_setting(name)
	)
...
static func get_cargo_package_directories() -> PackedStringArray:
	return _get_setting(
		_CARGO_PACKAGE_DIRECTORIES, PackedStringArray(), false, TYPE_PACKED_STRING_ARRAY
	)
...
static func get_enable_build_before_run() -> bool:
	return _get_setting(
		_ENABLE_BUILD_BEFORE_RUN, _ENABLE_BUILD_BEFORE_RUN_INITIAL_VALUE, true, TYPE_BOOL
	)
